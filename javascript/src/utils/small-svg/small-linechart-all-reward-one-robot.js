...
export default class SmallLineChartReward extends SmallLineChartSVG {
            ...
            .on('end', (event) => {
                if (event.selection) {
                    ...
                    if (x1 - x0 > 1) {
                        ...
                        this.points = [];
                        for (const key of this.rewardLabels) {
            ...
            this.path
                .style('stroke', ({ z }) =>
                    z === globalVariables.mouseOverObs
                        ...
                        : globalVariables.checkedRewards.includes(z)
    ...
    initMovement() {
        ...
        // y min and max
        for (const idx in this.rewardLabels) {
            this.all_y[this.rewardLabels[idx]] = this.movement.map((d) =>
                parseFloat(d[this.rewardLabels[idx]]),
            );
        }
    ...
    drawByX() {
        ...
        // keys in checkedRewards are blue, others are grey
        this.path
            .style('stroke', ({ z }) =>
                z === globalVariables.mouseOverObs
                    ? globalVariables.lineColors.mouseOver
